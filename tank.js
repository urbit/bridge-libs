...
import { WALLET_TYPES } from './wallet';
...
const remainingTransactions = (baseUrl, point) => {
  if (typeof point === 'string') point = Number(point);
  return sendRequest(baseUrl + '/point', { point: point });
};

const fundTransactions = (baseUrl, signedTxs) => {
  return sendRequest(baseUrl + '/request', { txs: signedTxs });
};

// resolves when address has at least cost balance
// returns true if gas tank was used, false otherwise
// baseUrl: location of the gas tank to use
...
const ensureFundsFor = async (
  ...
  try {
    ...
    if (point !== null) {
      const fundsRemaining = await remainingTransactions(baseUrl, point);
    ...
    const res = await fundTransactions(baseUrl, signedTxs);
    if (!res.success) {
